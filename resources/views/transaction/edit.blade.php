...
<div class="container">
  <form method="POST" action="{{ route('listtransaksi.update', $listtransaksi->id) }}">
    @csrf
    @method('PUT')
    <!-- Form input menu -->
    <div class="row mb-3">
      <div class="col-md-5">
        ...
        <select id="food_select" class="form-control">
          ...
          @foreach($menu as $c)
            <option value="{{ $c->id }}" data-name="{{ $c->name }}" data-price="{{ $c->price }}" @if($selectedFoods->contains('id', $c->id)) selected @endif>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="total_display" value="{{$listtransaksi->total_price}}" readonly>
    </div>

    <div class="form-group">
      <label>Nama Pelanggan</label>
      <input type="text" name="customer_name" class="form-control" value="{{$listtransaksi->customer_name}}"required>
    </div>
